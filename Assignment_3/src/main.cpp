...
double ray_sphere_intersection(const Vector3d &ray_origin, const Vector3d &ray_direction, int index, Vector3d &p, Vector3d &N)
{
    ...
    const Vector3d originCenter = ray_origin - sphere_center;
    double b = 2.0 * (ray_direction.dot(originCenter));
    double c = originCenter.dot(originCenter) - (sphere_radius * sphere_radius);
...
double ray_parallelogram_intersection(const Vector3d &ray_origin, const Vector3d &ray_direction, int index, Vector3d &p, Vector3d &N)
{
    ...
    Vector3d uv_dot = pgram_u.dot(pgram_v);
    double denom = uv_dot.dot(uv_dot);
    
    float d = uv_dot.dot(pgram_origin);

    t = (uv_dot.dot(ray_origin)+d);
    if (t < 0)
    {
        return -1;
    }
